...
- `[f,g,h].reduce((a,b) => (...args: any) => a(b(...args)))`
  ...
  - let's use `i` to denote the function `(...args) => f(g(...args))`, then we have `(i, h) => (...args) => i(h(...args))`, which when we plug `(...args) => f(g(...args))` back gives `(...args) => f(g(h(...args)))`

  - As there are no more functions left, the final return value is the function `(...args) => f(g(h(...args)))`
...
Finally, the wrapped `dispatch` is used to overwrite the `dispatch` method in `store`. 
...
This completes the `applyMiddleware` function:
...
`applyMiddleware` demonstrates great software engineering philosophy, where a package solves some core problems really well, avoids trying to do everything, delegates responsibility to allow custom functionality to be added. 



Thank you again for your taking your time to read the post. In the next post, I will be writing about how reducers are actually combined into a root reducer by `combineReducers`. 
